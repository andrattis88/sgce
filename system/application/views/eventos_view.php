 ...
 *
 * Utilizada para Listar eventos cadastrados
 *
...
<div class="botoes_left">
    <button onclick="parent.location='<?= base_url()?>eventos/novo'" type="button" id="botao_novo">
...
<table width="100%" border='0' class="center_table" id="data_table">
    <tr class="linha_par">
        <td width="80px"><b><a href="<?=base_url().'eventos/ordenar/codigo'?>">C&oacute;digo</a></b></td>
        <td><b><a href="<?=base_url().'eventos/ordenar/nome'?>">Nome</a></b></td>        
    ...
    <?php if (!@$mensagem): ?>
        ...
        <tr class='<?=$cor?>' id="linha_<?=$i?>" onmouseover="overHighLight('<?=$i?>')"
            ...
            <td width="30">
                <center>
                    <a href="<?= base_url()?>eventos/editar/<?=$row->id_evento?>"
            ...
            <td width="30">
                <center>
                    <a href="#"
                    onclick="javascript:confirmaExclusao('<?= base_url()?>eventos/excluir/<?=$row->id_evento?>','de <?=$row->nm_evento?>')"
